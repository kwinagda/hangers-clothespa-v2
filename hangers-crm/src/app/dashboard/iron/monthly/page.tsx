...
export default function DailyIronMonthlyPage() {
  ...
  const rowDraftBills = (row:any) => (row.bills || []).filter((bill:any) => String(bill.status || '').toUpperCase() === 'DRAFT' && billBalance(bill) > 0.005)
  const rowActionLabel = (row:any) => {
    ...
    return row.bills?.length ? 'Manage billing' : 'Review billing'
  }
  ...
  return <div className="monthly-page">
    ...
    <section className="summary-strip">
      <div><span>Customers</span><strong>{payload?.summary?.customers || 0}</strong></div><div><span>Total clothes</span><strong>{payload?.summary?.totalPieces || 0}</strong></div><div><span>Estimated value</span><strong>{money(payload?.summary?.totalAmount)}</strong></div><div><span>Unbilled</span><strong>{money(payload?.summary?.unbilledAmount)}</strong></div>
    </section>
