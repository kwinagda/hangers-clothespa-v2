...
const billVoid=(bill:any)=>String(bill?.status||'').toUpperCase()==='VOID'
...
export default function IronBillingWorkspace({month,rows,action,onClose,onChanged}:Props){
  ...
  const single=workspaceRows.length===1&&!action
  const [loading,setLoading]=useState(true)
  const [busy,setBusy]=useState('')
  const [billsByCustomer,setBillsByCustomer]=useState<Record<string,any[]>>({})
  const [lastGeneratedBillId,setLastGeneratedBillId]=useState('')
  ...
  useEffect(()=>{setWorkspaceRows(rows)},[rows])

  const load=useCallback(async()=>{
    setLoading(true)
    try{
      const customerIds=rows.map(row=>row.customer.id)
      const [billResponses,metadataResponse,summaryResponse]=await Promise.all([
        Promise.all(rows.map(async(row)=>({id:row.customer.id,response:await ironAPI.getBills(row.customer.id)}))),
        metadataAPI.getAll(),
        ironAPI.getMonthlySummary(month).catch(()=>null),
      ])
      ...
      const freshRows=asArray(summaryResponse?.data?.customers||summaryResponse?.customers).filter((row:any)=>customerIds.includes(row.customer?.id))
      if(freshRows.length) {
        const freshById=new Map(freshRows.map((row:any)=>[row.customer.id,row]))
        setWorkspaceRows(current=>current.map(row=>freshById.get(row.customer.id)||row))
      }
  ...
  const openBills=useCallback((row:any)=>monthBills(row).filter((bill:any)=>!billVoid(bill)&&!billPaid(bill)),[monthBills])
  ...
  const generate=async(row:any)=>{
    ...
    try{
      const response=await ironAPI.generateBill({customerId:row.customer.id,billingPeriodStart:`${month}-01`,...notes})
      const bill=response?.bill||response?.data?.bill
      if(bill?.id)setLastGeneratedBillId(bill.id)
      toast.success('Bill generated. Review it, then send on WhatsApp.')
      setNotes({carryForwardNotes:'',notes:''})
      await refresh()
    }
  ...
  const send=async(bill:any)=>{
    ...
    try{await ironAPI.sendBill(bill.id);if(bill.id===lastGeneratedBillId)setLastGeneratedBillId('');toast.success('Bill queued on WhatsApp');await refresh()}
    catch(error:any){toast.error(error.message||'Failed to send bill')}
    finally{setBusy('')}
  }
  const voidBill=async(bill:any)=>{
    const reason=window.prompt(`Reason for voiding ${bill.billNumber}`)?.trim()
    if(!reason||reason.length<3)return toast.error('A void reason is required')
    setBusy(`void-bill-${bill.id}`)
    try{await ironAPI.voidBill(bill.id,{reason});toast.success('Bill voided; clothes are unbilled again');await refresh()}
    catch(error:any){toast.error(error.message||'Failed to void bill')}
    finally{setBusy('')}
  }
  ...
  const sendGeneratedBulk=async()=>{
    if(busy)return
    setBusy('bulk-send-generated');setResults([])
    const next:Result[]=[]
    for(const row of workspaceRows){
      const drafts=unsentBills(row)
      if(!drafts.length){next.push({key:row.customer.id,name:row.customer.name,ok:false,message:'No draft bill to send'});continue}
      try{
        for(const bill of drafts)await ironAPI.sendBill(bill.id)
        next.push({key:row.customer.id,name:row.customer.name,ok:true,message:drafts.length===1?'Bill queued':'Bills queued'})
      }catch(error:any){next.push({key:row.customer.id,name:row.customer.name,ok:false,message:error.message||'Send failed'})}
      setResults([...next])
    }
    setBusy('');await refresh()
  }
  ...
  function SingleView({row}:{row:any}){
    ...
    const lastGeneratedBill=bills.find((bill:any)=>bill.id===lastGeneratedBillId&&!billVoid(bill))
    return <div className="body"><section className="summary"><div><span>Clothes</span><strong>{row.totalPieces}</strong></div><div><span>Month value</span><strong>{money(row.totalAmount)}</strong></div><div><span>Unbilled</span><strong>{money(row.unbilledAmount)}</strong></div></section>
      {lastGeneratedBill&&<section className="card generated-card"><div className="card-title"><h3>Bill generated</h3><span className="badge draft">{String(lastGeneratedBill.status||'DRAFT').toUpperCase()}</span></div><p className="flow-note">Review the bill below. It will not go to the customer until you click Send bill.</p></section>}
  ...
  function BulkView(){
    ...
    const eligibleCount=workspaceRows.filter(row=>eligibility(row,action).ok).length
    const generatedDraftCount=action==='GENERATE'?workspaceRows.reduce((sum,row)=>sum+unsentBills(row).length,0):0
    return <>
      <div className="body"><div className="bulk-list">{workspaceRows.map(row=>{
      ...
      <footer className="bulk-footer"><span>{action==='GENERATE'&&generatedDraftCount?`${generatedDraftCount} draft bill${generatedDraftCount===1?'':'s'} ready to send`:`${eligibleCount} of ${workspaceRows.length} ready`}</span><div className="bulk-footer-actions">{action==='GENERATE'&&generatedDraftCount>0&&<button className="secondary" disabled={!!busy} onClick={sendGeneratedBulk}>{busy==='bulk-send-generated'?<><Loader2 className="crm-spin" size={14}/> Sending…</>:<><Send size={14}/> Send generated bills</>}</button>}<button className="primary" disabled={!eligibleCount||!!busy} onClick={runBulk}>
