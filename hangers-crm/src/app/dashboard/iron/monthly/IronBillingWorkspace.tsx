...
export default function IronBillingWorkspace({month,rows,action,onClose,onChanged}:Props){
  ...
  function BillCard({bill}:{bill:any}){
    ...
    return <section className="card"><div className="card-title"><div><h3>{bill.billNumber}</h3><small>{bill.invoice?.invoiceNumber||'Invoice linked on generation'}</small></div><span className="badge">{isVoid?'Void':paid?'Paid':String(bill.status||'Draft')}</span></div><div className="metrics"><div className="metric"><span>Pieces</span><strong>{bill.totalPieces}</strong></div><div className="metric"><span>Total</span><strong>{money(bill.totalAmount)}</strong></div><div className="metric"><span>Paid</span><strong>{money(bill.invoice?.paidAmount??bill.paidAmount)}</strong></div><div className="metric"><span>Balance</span><strong>{isVoid?'Void':money(balance)}</strong></div></div>
      {isVoid&&<div className="void-note">Bill voided. Linked clothes are available for fresh billing.</div>}
      {!isVoid&&<div className="actions">{!paid&&<button className="primary" disabled={!!busy} onClick={()=>send(bill)}><Send size={13}/> {busy===`send-${bill.id}`?'Sending…':String(bill.status).toUpperCase()==='DRAFT'?'Send bill':'Send again'}</button>} {!paid&&<button className="danger" disabled={!!busy} onClick={()=>voidBill(bill)}>{busy===`void-bill-${bill.id}`?'Voiding…':'Void bill'}</button>}</div>}
