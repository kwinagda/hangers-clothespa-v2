...
import { campaignsAPI, automationsAPI } from '@/lib/api'
type Tab = 'campaigns'|'automations'
const TRIGGERS = ['ORDER_PLACED','ORDER_READY','PAYMENT_DUE','ORDER_DELIVERED']
const AUDIENCES = ['ALL','VIP','CORPORATE','REGULAR','NEW','INACTIVE']
export default function MarketingPage() {
  ...
  return (
    <div style={{padding:'32px 36px',maxWidth:900,margin:'0 auto',...s}}>
      <h1 style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:26,color:'#023c62',marginBottom:24}}>Marketing & Automations</h1>
      ...
      {tab==='campaigns'&&<div>
        ...
        <div style={{display:'flex',flexDirection:'column' as const,gap:12}}>
          ...
          campaigns.map((c:any)=><div key={c.id} style={{background:'#fff',borderRadius:12,border:'1px solid #e8f0f7',padding:20}}>
            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16}}>
              <div style={{flex:1}}>
                <div style={{fontWeight:700,color:'#023c62',marginBottom:4}}>{c.name}</div>
                <div style={{fontSize:13,color:'#6b7fa3',marginBottom:8}}>Audience: {c.audience}</div>
      ...
      {tab==='automations'&&<div>
        ...
        <div style={{display:'flex',flexDirection:'column' as const,gap:12}}>
          ...
          automations.map((a:any)=><div key={a.id} style={{background:'#fff',borderRadius:12,border:'1px solid #e8f0f7',padding:20}}>
            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16}}>
              <div style={{flex:1}}>
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                  <span style={{fontWeight:700,color:'#023c62'}}>{a.name}</span>
                  <span style={{padding:'2px 8px',background:'#dbeafe',color:'#1e40af',borderRadius:4,fontSize:11,fontWeight:600}}>{a.trigger}</span>
      ...
      {showCamp&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50}}>
        <div style={{background:'#fff',borderRadius:16,padding:24,width:'100%',maxWidth:440,boxShadow:'0 20px 60px rgba(0,0,0,0.15)'}}>
          <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:18,marginBottom:20}}>New WhatsApp Campaign</h2>
          <div style={{display:'flex',flexDirection:'column' as const,gap:14}}>
            ...
            <div><label style={{fontSize:12,color:'#6b7fa3',display:'block',marginBottom:6}}>Audience</label><select value={cf.audience} onChange={e=>setCf({...cf,audience:e.target.value})} style={{width:'100%',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 12px',fontSize:13}}>{AUDIENCES.map(a=><option key={a} value={a}>{a}</option>)}</select></div>
      ...
      {showAuto&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50}}>
        <div style={{background:'#fff',borderRadius:16,padding:24,width:'100%',maxWidth:440,boxShadow:'0 20px 60px rgba(0,0,0,0.15)'}}>
          <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:18,marginBottom:20}}>New Automation</h2>
          <div style={{display:'flex',flexDirection:'column' as const,gap:14}}>
            ...
            <div><label style={{fontSize:12,color:'#6b7fa3',display:'block',marginBottom:6}}>Trigger</label><select value={af.trigger} onChange={e=>setAf({...af,trigger:e.target.value})} style={{width:'100%',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 12px',fontSize:13}}>{TRIGGERS.map(t=><option key={t} value={t}>{t.replace(/_/g,' ')}</option>)}</select></div>
