...
export default function PublicSiteShell({ profile, children }: { profile: PublicSiteProfile; children: React.ReactNode }) {
  ...
  return <div className="dw-root">
    ...
    <style>{`
      ...
      @media(max-width:800px){.dw-links,.dw-book{display:none}.dw-mobile-toggle{display:grid}.dw-mobile-nav{position:fixed;inset:110px 16px auto;z-index:59;display:grid;padding:10px;border:1px solid var(--line);border-radius:10px;background:#fff;box-shadow:0 20px 44px rgba(2,60,98,.2)}.dw-mobile-nav a{padding:12px 11px;border-bottom:1px solid #edf3f7;color:var(--text);font-size:14px;font-weight:650}.dw-mobile-nav a:last-child{border:0}.dw-cta-row{align-items:flex-start;flex-direction:column}.dw-footer-bottom{flex-direction:column}}
    ...
    <header className="dw-header"><nav className="dw-container dw-nav" aria-label="Primary"><Link href="/" onClick={() => setMenuOpen(false)}><img className="dw-logo" src={LOGO_BLUE_URL} alt={profile.businessName} /></Link><div className="dw-links">{links.map(([href,label])=><Link key={href} className={pathname===href?'active':''} href={href}>{label}</Link>)}</div><Link className="dw-book" href="/book-pickup">Book a pickup</Link><button className="dw-mobile-toggle" type="button" aria-label={menuOpen?'Close menu':'Open menu'} aria-expanded={menuOpen} onClick={()=>setMenuOpen(v=>!v)}>{menuOpen?<X size={20}/>:<Menu size={20}/>}</button>{menuOpen&&<div className="dw-mobile-nav">{links.map(([href,label])=><Link key={href} href={href} onClick={()=>setMenuOpen(false)}>{label}</Link>)}<Link href="/book-pickup" onClick={()=>setMenuOpen(false)}>Book a pickup</Link></div>}</nav></header>
