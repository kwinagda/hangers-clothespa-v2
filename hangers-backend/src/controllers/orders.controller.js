  ...
  try {
    ...
    if (amountNum > 0) {
      prisma.order.findFirst({
        ...
        if (!o) return;
        const sent = await sendPaymentReceivedMessage(o, amountNum, normalizedMethod);
        if (!sent) console.warn(`[Whatomate] Payment notification not sent for order ${o.orderNumber}`);
      }).catch((err) => {
